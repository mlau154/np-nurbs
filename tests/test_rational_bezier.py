...
@pytest.fixture
def w_curve() -> np.ndarray[tuple[int,], np.dtype[np.float64]]:
    return np.random.uniform(low=0.01, high=10.0, size=(10,))


@pytest.fixture
def w_surf() -> np.ndarray[tuple[int, int], np.dtype[np.float64]]:
...
def test_rational_bezier_curve_eval_grid(
        ...
        w_curve: np.ndarray[tuple[int,], np.dtype[np.float64]]
        ):
...
def test_rational_bezier_surf_eval_grid(
        ...
        w_surf: np.ndarray[tuple[int, int], np.dtype[np.float64]]
        ):
